...
def KMC_step(
    ...
    # field allows us to observe the activity of other receptors (up, down, left, right) in lattice
    field = J * (
        np.roll(a, 1, axis=0)
        + np.roll(a, -1, axis=0)
        + np.roll(a, 1, axis=1)
    ...
    # determine the time to the next event - the rates determine time until next event!! like in 2-node
    t = 1 / R * np.log(1 / r1)
    ...
    # return new lattice with receptors in 0, -1, 1 state
    return a, t

# activity estimate --> (Fig 1C) --> after simulation, what percentage of receptors are in which state (0, -1, 1)
...
def calc_dwell_and_switching_times(
    ...
    print("Done calculating final state")
...
def save_results(results, path="simulation_results.npz"):
